...
from scout.typesafe.weights import UncertainBand, WeightSet, weight_set_version
...
def extract_features(answers: Answers) -> dict[str, float]:
    ...
    for question_id, answer in answers.answers.items():
        if "/" in question_id:
            raise ValueError(f"question id contains reserved '/': {question_id}")
        if isinstance(answer, ProbabilityAnswer):
            features[question_id] = float(answer.probability)
        elif isinstance(answer, ScoreAnswer):
            for level in answer.levels:
                if "/" in level.level:
                    raise ValueError(f"score level contains reserved '/': {level.level}")
...
def cost_policy(c_fp: float, c_fn: float) -> tuple[float, UncertainBand]:
    ...
    scale = max(c_fp, c_fn)
    scaled_fp = c_fp / scale
    scaled_fn = c_fn / scale
    threshold = scaled_fp / (scaled_fp + scaled_fn)
