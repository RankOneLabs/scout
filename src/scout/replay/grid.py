...
def _run_script(grid: GridDocument, sweeps: Sequence[GridSweep], *, grid_dir_from_out: str) -> str:
    ...
    plan hash, execute. Paths in the grid resolve relative to the grid
    document; the script is written next to the sweeps and refers back."""
    lines = [
        "#!/bin/bash",
        ...
        (
            f'GRID_DIR="{grid_dir_from_out}"'
            if os.path.isabs(grid_dir_from_out)
            else f'GRID_DIR="$HERE/{grid_dir_from_out}"'
        ),
        'SCOUT=${SCOUT:-"scout"}',
        "",
    ]
    for sweep in sweeps:
        ...
        if backend.identity_config:
            env_parts.append(
                f'SCOUT_MODEL_IDENTITY_CONFIG="$(cat "$GRID_DIR/{backend.identity_config}")"'
            )
        ...
        args = [
            ...
            sweep.name,
            "--task-config",
            f'"$GRID_DIR/{project.task_config}"',
            "--sweep-file",
            f'"$HERE/{sweep.file}"',
            "--pricing-catalog",
            f'"$GRID_DIR/{backend.pricing_catalog}"',
        ]
        if project.dossier_root:
            args += ["--dossier-root", shlex.quote(project.dossier_root)]
        cmd = f'{env_prefix}"$SCOUT" ' + " ".join(args)
        lines += [
            f'echo "PLAN {sweep.name} $(date -u +%FT%TZ)"',
            f'sha=$({cmd} 2>/dev/null | grep "canonical plan sha256" | grep -oE "[0-9a-f]{{64}}")',
            f'if [ -z "$sha" ]; then echo "NO SHA {sweep.name}"; else',
            f'  {cmd} --authorize-plan-sha256 "$sha" --execute-paid-replay',
            f'  echo "EXIT {sweep.name}=$? $(date -u +%FT%TZ)"',
            "fi",
            "",
        ]
